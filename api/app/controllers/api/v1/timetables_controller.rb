...
class Api::V1::TimetablesController < ApplicationController
  ...
  def internal(datetime = DateTime.now.to_s)
    ...
    @searchType = params[:search] ? params[:search] : "default"
    ...
    if dateset
      ...
      for c in course_ids do
        @timetables << {
          ...
          timetables: dateset ? 
            @searchType == "arrival" ? 
              DateSet.find_by(date: @date).timetable_set.timetables.where("course_id = ? AND arrival_time <= ?", c.course_id, @time ).order(departure_time: "DESC").limit(2)
              : @searchType == "first" ?
  ...
  def lastupdate()
    ...
    render json: { success: true, datetime: last_dateset.updated_at }, status: :ok if last_dateset
  rescue => e
    render json: { success: false, errors: '500 internal server error. Please contact the administrator.' }, status: :internal
  end
